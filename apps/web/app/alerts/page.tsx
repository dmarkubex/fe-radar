        ...
        {Object.entries(grouped).map(([level, group]) => {
          ...
          return (
            <section key={level}>
              ...
              {group.map((item) => (
                <BigAlert key={item.id} item={item} level={level} />
              ))}
            </section>
          );
        })}
...
function BigAlert({
  ...
  return (
    <article
      className={`mb-2.5 grid grid-cols-[1fr_220px] items-start gap-7 border bg-surface px-5 py-[18px] max-[1100px]:grid-cols-1 ${meta.card}`}
    >
      ...
      <aside className="flex min-h-full flex-col gap-3 border-l border-hairline pl-6 max-[1100px]:border-l-0 max-[1100px]:border-t max-[1100px]:pl-0 max-[1100px]:pt-4">
        ...
        <div className="mt-auto flex gap-1.5">
          ...
          <button
            className="border border-border bg-surface px-3 py-2 text-xs text-fg-muted hover:bg-bg-deep"
            type="button"
          >
            忽略
          </button>
