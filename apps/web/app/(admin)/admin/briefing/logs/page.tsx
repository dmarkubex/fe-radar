...
import { briefingHolidays, commodityBriefings, commodityQuotes, getDb, sources } from "@fe-radar/db";
...
function StatusBadge({ status }: { status: string }): React.JSX.Element {
  ...
  return <span className={`font-mono text-[10px] uppercase tracking-[0.8px] ${meta.color}`}>{meta.label}</span>;
}
...
async function buildPageDiagnostic(db: ReturnType<typeof getDb>, rows: BriefingListRow[]): Promise<PageDiagnostic | null> {
  const now = dayjs().tz(APP_TIMEZONE);
  const todayStr = now.format("YYYY-MM-DD");
  const latest = rows[0] ?? null;
  const quoteStatsDate = latest?.genStatus === "failed" ? latest.briefingDate : todayStr;

  const [enabledQuoteSourceRows, holidayRows, latestQuoteRows, quoteStatsRows] = await Promise.all([
    db
      .select({ value: sql<number>`count(*)::int` })
      .from(sources)
      .where(and(eq(sources.fetcherType, "quotes"), eq(sources.enabled, true))),
    db.select({ name: briefingHolidays.name }).from(briefingHolidays).where(eq(briefingHolidays.holidayDate, todayStr)).limit(1),
    db.select({ observedAt: commodityQuotes.observedAt }).from(commodityQuotes).orderBy(desc(commodityQuotes.observedAt)).limit(1),
    db
      .select({
      ...
      .where(sql`DATE(${commodityQuotes.observedAt} AT TIME ZONE 'Asia/Shanghai') = ${quoteStatsDate}::date`)
  ]);
  ...
  if (enabledQuotesSources === 0) {
    return {
      tone: "warn",
      title: "行情信源未启用",
      detail: "当前没有 enabled=true 的 quotes 信源；系统不会自动启用信源，需要后台确认后手动开启。"
    };
  }
  ...
  if (isMockMode()) {
    return {
      ...
      diagnostic: {
        tone: "muted",
        title: "演示模式暂无简报",
        detail: "当前运行在 mock 数据模式，铜锂行情简报需要连接业务数据库后展示。"
      }
    };
  }
  ...
  try {
    ...
    return {
      ...
      diagnostic: {
        tone: "warn",
        title: "简报数据暂不可用",
        detail: "数据库连接或简报表结构暂不可用；页面已保留入口，恢复后会自动显示最近 60 期简报。"
      }
    };
  }
}

export default async function BriefingLogsPage(): Promise<React.JSX.Element> {
  const { rows, diagnostic } = await loadBriefingPageData();

  const RETENTION_DAYS = 90;
  function isExpired(dateStr: string): boolean {
    return dayjs(dateStr).tz(APP_TIMEZONE).isBefore(dayjs().tz(APP_TIMEZONE).subtract(RETENTION_DAYS, "day").startOf("day"));
  }
  ...
  return (
    <PageFrame size="wide">
      <PageHeader
        eyebrow="管理 · 简报 · LOGS"
        title="简报生成日志"
        description="查看最近 60 期铜锂简报的生成状态、失败原因和文档保留情况。"
      />

      {diagnostic && (
        <div className={`mb-4 border px-4 py-3 ${diagnosticClass(diagnostic.tone)}`}>
          <div className="text-sm font-medium text-fg">{diagnostic.title}</div>
          <div className="mt-1 text-xs leading-5 text-fg-soft">{diagnostic.detail}</div>
        </div>
      )}

      {rows.length === 0 ? (
        <div className="py-20 text-center text-sm text-fg-soft">暂无简报数据</div>
      ) : (
        <div className="flex flex-col divide-y divide-hairline border border-border rounded-[2px] overflow-hidden">
          {rows.map((row) => {
            ...
            const generatedAt = row.generatedAt instanceof Date ? dayjs(row.generatedAt).tz(APP_TIMEZONE).format("HH:mm") : "-";
            const canView = row.genStatus === "succeeded" || row.genStatus === "degraded";

            return (
              <div
                key={row.id}
                className="group flex flex-col items-stretch justify-between gap-4 bg-surface px-5 py-4 transition-colors hover:bg-bg-deep shell:flex-row shell:items-center"
              >
                {/* Left: date + status */}
                    ...
                    <div className="mt-0.5 flex flex-wrap items-center gap-2">
                      ...
                      {row.genStatus === "failed" && row.genError && (
                        <details className="min-w-0 font-mono text-[10px] text-danger">
                          ...
                          <p className="mt-1 max-w-[48ch] break-words leading-4" title={row.genError}>
                            {row.genError}
                          </p>
                        </details>
                      )}
                      {expired && row.docxPath && <span className="font-mono text-[10px] text-fg-soft">· docx 已过保留期</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-3">
                  <RegenerateButton briefingId={row.id} genStatus={row.genStatus} isAdmin />
                  {canView ? (
                    <Link
                    ...
                    <span className="text-[11px] text-fg-soft font-mono">{row.genStatus === "pending" ? "生成中…" : "不可用"}</span>
                  )}
                </div>
              </div>
            );
