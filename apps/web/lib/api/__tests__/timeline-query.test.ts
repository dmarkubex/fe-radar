...
import { encodeTimelineCursor, resolveTimelinePaginationPlan, type TimelineItemDto } from "../timeline-query";

function byPublishedDesc(a: { id: number; publishedAt: string }, b: { id: number; publishedAt: string }): number {
  return dayjs(b.publishedAt).valueOf() - dayjs(a.publishedAt).valueOf() || b.id - a.id;
}
...
describe("timeline query pagination plan", () => {
  it("uses publishedAt for the default order and keyset cursor", () => {
    ...
    const cursor = encodeTimelineCursor({ id: 11, publishedAt, scoredAt }, plan.keysetAxis);

    expect(plan).toEqual({ orderBy: "publishedAt", keysetAxis: "publishedAt" });
    expect(decodeCursor(cursor ?? undefined)).toEqual({ at: publishedAt.toISOString(), id: 11 });
  });
  ...
  it("keeps curated order on qualityScore and keyset on scoredAt", () => {
    ...
    const cursor = encodeTimelineCursor({ id: 12, publishedAt, scoredAt }, plan.keysetAxis);

    expect(plan).toEqual({ orderBy: "qualityScore", keysetAxis: "scoredAt" });
    expect(decodeCursor(cursor ?? undefined)).toEqual({ at: scoredAt.toISOString(), id: 12 });
  });
});
...
describe("mock timeline keyset behavior", () => {
  it("paginates by publishedAt/id without duplicates or gaps across a same-publishedAt boundary", () => {
    ...
    const second = mockFetchTimeline({ limit: 1, cursor: first.nextCursor ?? undefined });
    expect(second.items.map((item) => item.id)).toEqual([29]);

    const expected = [...mockTimelineItems].sort(byPublishedDesc).map((item) => item.id);
  ...
  it("orders search results by publishedAt/id and keeps cursor pagination gap-free", () => {
    const expected = mockTimelineItems
      .filter((item) => `${item.title} ${item.summaryZh ?? ""} ${item.sourceName}`.toLowerCase().includes("电缆"))
      .sort(byPublishedDesc)
      .map((item) => item.id);

    const actual = collectTimelinePages({ limit: 3, search: "电缆" }).map((item) => item.id);
  ...
  it("contains at least three days with at least two populated time buckets per day", () => {
    ...
    for (const item of rows) {
      const dayKey = dayjs(item.publishedAt).tz(APP_TIMEZONE).format("YYYY-MM-DD");
