...
test.afterAll(async ({ request }) => {
  ...
  await request.post("/api/auth/callback/credentials", {
    form: {
      ...
      callbackUrl: "/admin/briefing/targets"
    },
    maxRedirects: 0
  });
...
test("admin 可以登录并进入 briefing targets 后台", async ({ page }) => {
...
test("admin 可以通过 UI 新建推送目标", async ({ page }) => {
...
test("admin 可以编辑推送目标名称", async ({ page }) => {
  const updatedName = `E2E 已编辑 ${Date.now()}`;
  await login(page, adminUsername, adminPassword);
  const target = await createTargetViaApi(page, `E2E 编辑 ${Date.now()}`);
  await page.goto("/admin/briefing/targets");

  // Wait for the row to appear in the table
  await page.waitForSelector(`[data-testid="target-row-${target.id}"]`);

  const row = page.getByTestId(`target-row-${target.id}`);
...
test("sign_secret 字段类型为 password 且 API 响应中掩码为 ***", async ({ page }) => {
...
test("admin 删除推送目标需软确认（输入名称）", async ({ page }) => {
  await login(page, adminUsername, adminPassword);
  const target = await createTargetViaApi(page, `E2E 删除 ${Date.now()}`);
  await page.goto("/admin/briefing/targets");

  await page.waitForSelector(`[data-testid="target-row-${target.id}"]`);

  // Open delete modal
  await page.getByTestId(`delete-btn-${target.id}`).click();
  ...
  // Retrieve the name shown in the table (it was created by API with a name we know)
  const listRes = await page.request.get("/api/briefing/targets");
  const { items } = await listRes.json() as { items: Array<{ id: number; name: string }> };
  const targetName = items.find((i) => i.id === target.id)?.name ?? "";
  ...
  await expect(page.getByTestId(`target-row-${target.id}`)).not.toBeVisible();
...
test("测试推送按钮触发 POST .../test 并显示 toast", async ({ page }) => {
  await login(page, adminUsername, adminPassword);
  const target = await createTargetViaApi(page, `E2E 测试推送 ${Date.now()}`);
  await page.goto("/admin/briefing/targets");
  ...
  await page.getByTestId(`test-push-${target.id}`).click();
...
test("viewer 访问 admin briefing/targets 返回 403", async ({ page }) => {
