...
export async function CuratedContent({ category }: { category: string }): Promise<React.JSX.Element> {
    ...
    <>
      {heroItem ? (
        <section className="hero-pick grid gap-0 sm:grid-cols-[1fr_280px]">
          <div className="border border-hairline bg-surface p-6">
            <div className="flex flex-wrap items-center gap-2 text-xs text-fg-muted">
              ...
              <span className="font-mono text-[10px] text-fg-soft">
                {heroItem.scoredAt
                  ? new Date(heroItem.scoredAt).toLocaleDateString("zh-CN", {
                      month: "2-digit",
                      day: "2-digit",
                      hour: "2-digit",
                      minute: "2-digit",
            ...
            <h2 className="mt-3 text-xl font-semibold leading-7 text-fg">{heroItem.title}</h2>
            {heroItem.summaryZh ? (
          ...
          <div className="signal-panel border border-l-0 border-hairline bg-bg-deep p-5">
            <p className="font-mono text-[13px] font-medium uppercase tracking-[1.4px] text-fg-soft">综合评分</p>
            ...
            <div className="mt-4 space-y-2">
              {["D1", "D2", "D3", "D4", "D5"].map((dim) => (
                <div key={dim} className="flex items-center gap-2">
                  <span className="w-6 font-mono text-[13px] text-fg-soft">{dim}</span>
                  <div className="h-1.5 flex-1 rounded-none bg-border">
                    <div
                      className="h-full rounded-none bg-accent"
                      style={{
                        width:
                          heroItem.qualityScore !== null ? `${Math.min(100, heroItem.qualityScore * 10)}%` : "0%",
                      }}
                    />
                  </div>
                </div>
      ...
      {tableItems.length > 0 ? (
        <section className="table-section">
          <h2 className="mb-3 font-mono text-[10px] font-medium uppercase tracking-[1.4px] text-fg-soft">同主题更多</h2>
                ...
                {tableItems.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="whitespace-nowrap font-mono text-[10px] text-fg-soft">
                      {item.scoredAt
                        ? new Date(item.scoredAt).toLocaleDateString("zh-CN", {
                            month: "2-digit",
                            day: "2-digit",
                            hour: "2-digit",
                            minute: "2-digit",
