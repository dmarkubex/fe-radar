...
import { enqueueWebsearchForEntities } from "../jobs/websearch-enqueue";
...
export async function handleNerJob(job: { data: PipelineJob }): Promise<void> {
  ...
  try {
    ...
    try {
      await enqueueWebsearchForEntities(
        c1c2Hits.map((hit) => ({ entityId: hit.entityId, entityName: hit.canonicalName })),
        { itemId, correlationId, conn, queue, logger },
      );
    } finally {
      await queue.close();
      await conn.quit();
