...
function textResponse(body: string): Awaited<ReturnType<typeof undiciFetch>> {
  return new Response(body) as unknown as Awaited<ReturnType<typeof undiciFetch>>;
}

async function fetchOk(options: { insecureTLS?: boolean; source?: string } = {}): Promise<CapturedFetchInit> {
  mockFetch.mockResolvedValueOnce(textResponse("ok"));
  await fetchTextWithPolicy("https://example.com/news", { timeoutMs: 1000, ...options });
...
describe("fetchTextWithPolicy dispatcher", () => {
  ...
  it("uses a TLS-relaxed Agent without proxy when insecureTLS is enabled", async () => {
    const init = await fetchOk({ insecureTLS: true, source: "电缆网 cableabc" });
  ...
  it("uses a TLS-relaxed ProxyAgent when proxy and insecureTLS are enabled", async () => {
    mocks.acquire.mockReturnValue({ id: "proxy-1", server: "http://proxy.example:8080", disabled: false, failCount: 0 });
    ...
    expect(mockProxyPool.release).toHaveBeenCalledWith(expect.objectContaining({ id: "proxy-1" }), true);
  });
