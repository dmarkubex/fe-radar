...
import { DuplicateEnabledSourceError, ProtectedSourceUrlError, updateSource } from "../repos/sources";
...
describe("sources schema", () => {
  ...
  it("records when an admin updates a source", async () => {
    ...
    const returning = vi.fn().mockResolvedValue([{ id: 7, adminTouchedAt: now }]);
    ...
    expect(set).toHaveBeenCalledWith({
      enabled: true,
      adminTouchedAt: now,
      adminSnapshot: expect.objectContaining({ queryChunks: expect.arrayContaining([JSON.stringify({ enabled: true })]) })
    });
  ...
  it("snapshots only an explicitly updated config", async () => {
    ...
    const where = vi.fn().mockReturnValue({ returning });
    const set = vi.fn().mockReturnValue({ where });
    const db = { update: vi.fn().mockReturnValue({ set }) } as unknown as DbClient;

    await updateSource(db, 7, { config });

    expect(set).toHaveBeenCalledWith(expect.objectContaining({
      config,
      adminSnapshot: expect.objectContaining({ queryChunks: expect.arrayContaining([JSON.stringify({ config })]) })
    }));
  });
  ...
  it("does not write an admin snapshot for unrelated fields", async () => {
    ...
    expect(set).toHaveBeenCalledWith(expect.objectContaining({ name: "Updated source" }));
  ...
  it("rejects changing a locked seed URL after its original URL was already changed", async () => {
    ...
    await expect(updateSource(db, 7, { url: "https://example.com/cu-again" }))
      .rejects.toBeInstanceOf(ProtectedSourceUrlError);
    expect(db.update).not.toHaveBeenCalled();
  });

  it("allows changing an ordinary source URL", async () => {
    const selectWhere = vi.fn().mockResolvedValue([{ url: "https://example.com/old", urlLocked: false }]);
    const from = vi.fn().mockReturnValue({ where: selectWhere });
    const returning = vi.fn().mockResolvedValue([{ id: 7, url: "https://example.com/new" }]);
    ...
    const updated = await updateSource(db, 7, { url: "https://example.com/new" });

    expect(updated?.url).toBe("https://example.com/new");
    expect(set).toHaveBeenCalledWith(expect.objectContaining({ url: "https://example.com/new" }));
  });

  it("allows resubmitting the unchanged protected URL", async () => {
    const protectedUrl = "https://hq.smm.cn/h5/cu";
    const selectWhere = vi.fn().mockResolvedValue([{ url: protectedUrl, urlLocked: true }]);
    const from = vi.fn().mockReturnValue({ where: selectWhere });
    ...
    await expect(updateSource(db, 7, { url: protectedUrl })).resolves.toMatchObject({ url: protectedUrl });
  ...
  it("rejects enabling a locked SMM source when the same group is already enabled", async () => {
    ...
    const siblingsWhere = vi.fn().mockResolvedValue([
      { id: 9, url: "https://example.com/other", fetcherType: "quotes", config: { adapter: "smm-hq", metric_keys: ["lc_main_close"] }, enabled: true },
      { id: 11, url: "https://example.com/cu-copy", fetcherType: "quotes", config: { adapter: "smm-hq", metric_keys: ["cu_main_close"] }, enabled: true },
      { id: 3, url: "https://hq.smm.cn/h5/cu", fetcherType: "quotes", config: {}, enabled: true }
    ]);
    const db = {
      select: vi.fn()
    ...
    const error = await updateSource(db, 7, { enabled: true }).catch((caught: unknown) => caught);
  ...
  it("allows enabling a locked SMM source when its sibling is disabled", async () => {
    ...
    await expect(updateSource(db, 7, { enabled: true })).resolves.toMatchObject({ enabled: true });
    expect(db.update).toHaveBeenCalledOnce();
  });
  ...
  it("does not apply the SMM sibling guard to an unrelated locked source", async () => {
    ...
    const set = vi.fn().mockReturnValue({ where: updateWhere });
    const db = {
      select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ where: currentWhere }) }),
      update: vi.fn().mockReturnValue({ set })
    } as unknown as DbClient;
