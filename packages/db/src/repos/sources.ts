...
export class SourceFetcherConfigMismatchError extends AppError {
  public constructor(fetcherType: string, configType: string | undefined) {
    super(
      "SOURCE_FETCHER_CONFIG_MISMATCH",
      `fetcherType "${fetcherType}" 与 config.type "${configType ?? "missing"}" 不一致`
    );
  }
}

export class SourceVerificationPolicyError extends AppError {
  public constructor() {
    super(
      "SOURCE_VERIFICATION_POLICY_PROTECTED",
      "该信源因 robots.txt 合规策略禁止抓取与复检，不能通过普通信源编辑解除"
    );
  }
}
...
function filtersToWhere(filters: SourceListFilters): SQL | undefined {
  ...
  if (typeof filters.enabled === "boolean")
    clauses.push(eq(sources.enabled, filters.enabled));
  return clauses.length > 0 ? and(...clauses) : undefined;
}

function smmLogicalGroup(row: {
  url: string;
  fetcherType: string;
  config: unknown;
}): "cu" | "lc" | null {
  ...
  if (Array.isArray(metricKeys) && metricKeys.includes("cu_main_close"))
    return "cu";
  if (Array.isArray(metricKeys) && metricKeys.includes("lc_main_close"))
    return "lc";
  return null;
}
...
export async function listEnabledByTier(
  db: DbClient,
  tier: SourceTier
): Promise<SourceRecord[]> {
  ...
  if (
    ...
    const currentConfig =
      current.config && typeof current.config === "object"
        ...
        : {};
    if (currentConfig.verificationBlocked === true && input.enabled === true) {
      throw new SourceVerificationPolicyError();
    }
    if (input.fetcherType !== undefined || input.config !== undefined) {
      const nextFetcherType = input.fetcherType ?? current.fetcherType;
      ...
      const nextConfigRecord =
        nextConfig && typeof nextConfig === "object"
          ...
          : {};
      if (
        currentConfig.verificationBlocked === true &&
        nextConfigRecord.verificationBlocked !== true
      ) {
        throw new SourceVerificationPolicyError();
      }
      const nextConfigType = nextConfigRecord.type;
      if (nextConfigType !== nextFetcherType) {
        throw new SourceFetcherConfigMismatchError(
          nextFetcherType,
          typeof nextConfigType === "string" ? nextConfigType : undefined
        );
      }
    }
    if (
      current?.urlLocked === true &&
      input.url !== undefined &&
    ...
    if (current?.urlLocked === true && input.enabled === true) {
      ...
      if (logicalGroup !== null) {
        const siblings = await db
          ...
          .from(sources)
          .where(and(eq(sources.urlLocked, true), ne(sources.id, id)));
        const conflicting = siblings
          .filter(
            (sibling) =>
              sibling.enabled === true &&
              smmLogicalGroup(sibling) === logicalGroup
          )
...
export async function markSourceFailure(
  ...
  await db
    ...
    .set({
      ...
      lastErrorAt:
        lastError === undefined ? undefined : lastError === null ? null : now
    })
    .where(eq(sources.id, id));
}
