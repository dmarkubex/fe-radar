...
function normalizeMockItem(item: {
  category: string;
  quality: number;
  ...
  return {
    ...
    quality: scaleScore(item.quality),
    scores: {
      d1: scaleScore(item.scores.d1),
      d2: scaleScore(item.scores.d2),
      d3: scaleScore(item.scores.d3),
      d4: scaleScore(item.scores.d4),
      d5: scaleScore(item.scores.d5)
    }
  };
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const allEntities =
      await sql`SELECT id, canonical_name, type, circle FROM entities`;
    ...
    const mockItems = [
      {
        title:
          "国家能源局发布《2026 储能并网技术导则》征求意见稿，9 月 1 日起施行",
        sourceName: "能源局官网",
        tier: "T1" as const,
        category: "policy",
        circle: "C2" as const,
        ...
        alertLevel: "L2",
        summaryZh:
          "新导则对储能并网频率响应、SOC 管理、二次设备配置提出量化要求。对自家智慧能源储能业务影响 ★★★。",
        eventType: "政策发布"
      },
      {
        ...
        alertLevel: "L1",
        summaryZh:
          "紫金电缆是竞品企业，安全事故可能影响其产能和订单交付，间接利好远东电缆业务线。",
        eventType: "安全事故"
      },
      {
        ...
        alertLevel: "L2",
        summaryZh:
          "自家公司直接中标项目，金额待公告。关注后续合同细节及交货周期。",
        eventType: "中标"
      },
      {
      ...
      },
      {
        ...
        alertLevel: "L3",
        summaryZh:
          "本批次新增海缆专项 18 亿，宝胜、亨通、东方电缆已报名；竞品供应链情报值得关注。",
        eventType: "招标"
      },
      {
        ...
        alertLevel: null,
        summaryZh:
          "LME + 国内现货同步上涨。对铜杆采购成本和电缆毛利的传导，建议供应链部门同步评估。",
        eventType: "价格异动"
      },
      {
        ...
        alertLevel: "L2",
        summaryZh:
          "新版规范条件提高了准入门槛，强化了质量追溯和环保要求。利好头部企业市场份额。",
        eventType: "政策发布"
      },
      {
      ...
      },
      {
        ...
        alertLevel: null,
        summaryZh:
          "报告预测 2026 年全社会用电量增长 5.5%，新能源装机占比将超 40%。",
        eventType: "行业报告"
      },
      {
        ...
        alertLevel: null,
        summaryZh:
          "液流电池技术路线在长时储能场景的经济性持续改善，与远东储能战略相关。",
        eventType: "技术突破"
      },
      {
      ...
      },
      {
        ...
        alertLevel: null,
        summaryZh:
          "锂电池回收新规对储能电池退役后的处理提出要求，间接影响储能系统全生命周期成本。",
        eventType: "政策征求意见"
      }
    ];
    ...
    for (let i = 0; i < mockItems.length; i++) {
      ...
      const [item] =
        await sql`INSERT INTO items (source_id, url, title, content, lang, published_at)
        VALUES (${sourceId}, ${`https://example.com/mock/${randomUUID()}`}, ${m.title}, ${`这是关于"${m.title}"的详细内容。这里是模拟的正文内容，用于前端测试。`}, 'zh', ${publishedAt})
        RETURNING id`;

      const normalized = normalizeMockItem(m);
      const isCurated = normalized.quality >= 50;
      ...
      await sql`INSERT INTO item_analysis (item_id, is_industry_related, summary_zh, d1_policy, d2_chain, d3_market, d4_tech, d5_business, quality_score, category, top_circle, is_curated, alert_level, alert_type, scored_at)
        VALUES (${item!.id}, true, ${m.summaryZh}, ${normalized.scores.d1}, ${normalized.scores.d2}, ${normalized.scores.d3}, ${normalized.scores.d4}, ${normalized.scores.d5}, ${normalized.quality}, ${normalized.category}, ${m.circle}, ${isCurated}, ${m.alertLevel}, ${m.alertType}, ${scoredAt})`;

      // link entities
      const matchedEntities = (
        allEntities as unknown as Array<{
          id: number;
        ...
        return (
          m.summaryZh.includes(e.canonicalName) ||
          m.title.includes(e.canonicalName) ||
          e.aliases.some(
            (a: string) => m.title.includes(a) || m.summaryZh.includes(a)
          )
        );
      });
    ...
    // ── 1 cluster ──
    const top2Items =
      await sql`SELECT i.id FROM items i JOIN item_analysis a ON a.item_id = i.id ORDER BY a.quality_score DESC LIMIT 2`;
    if (top2Items.length >= 2) {
      await sql`INSERT INTO clusters (event_type, lead_item_id) VALUES ('招标', ${top2Items[0]!.id}) RETURNING id`.then(
        async (clusterRows) => {
    ...
    await sql`INSERT INTO daily_reports (date, sections) VALUES (${today}, ${JSON.stringify(
      {
        ...
        hero_summary:
          "今日储能并网导则与线缆行业规范双发，铜价突破 78,400 创 18 月新高，紫金电缆火灾事故引发行业关注。自家公司中标国网江苏招标、参展国际线缆展。",
        briefs: [
          "储能并网导则 9 月施行",
          "紫金电缆车间火灾",
          "远东中标国网江苏",
        ...
        policy: {
          summary:
            "国家能源局发布储能并网技术导则征求意见稿，9 月 1 日起施行。工信部同步修订电线电缆行业规范条件，提高准入门槛。",
          lead: {
            title: "储能并网技术导则征求意见",
            summary: "对频率响应、SOC 管理提出量化要求",
            source: "能源局官网",
            time: "09:32",
            score: "7.2"
          },
        ...
        market: {
          summary:
            "电解铜均价上探 78,400 元/吨，LME + 国内现货同步上涨。对铜杆采购成本和电缆毛利的传导效应需持续评估。",
          lead: {
            title: "电解铜创新高",
            summary: "建议供应链部门同步评估",
            source: "第一财经",
          ...
          items: [{ title: "2026 年度电力供需形势分析", source: "中电联 · T1" }]
        },
        project: {
          summary:
            "南方电网 78 亿海缆招标启动，国网江苏一批次结果出炉，远东智慧能源成功中标。",
          lead: {
            title: "南网 78 亿海缆招标",
            summary: "新增海缆专项 18 亿",
            source: "电缆网",
        ...
        tech: {
          summary:
            "全钒液流电池成本再降 12%，固态电池中试线在合肥投产，储能技术路线多元化加速。",
          items: [
        ...
        company: {
          summary:
            "紫金电缆车间火灾事故、亨通光电 Q1 营收 +14.2%、远东智慧能源参展国际线缆展。",
          lead: {
            title: "紫金电缆火灾事故",
            summary: "无人员伤亡，可能影响竞品产能",
            source: "第一财经",
            time: "09:51",
            score: "6.0"
          },
